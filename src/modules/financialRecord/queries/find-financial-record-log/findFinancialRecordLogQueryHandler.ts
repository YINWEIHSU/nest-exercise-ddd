    ...
    >
{
  ...
  async execute(
    query: FindFinancialRecordLogQuery,
  ): Promise<any> {
    ...
    const applicationFormMap = Object.fromEntries(applicationForms.map(i => [i.id, i.name]));
    const counterpartyMap = Object.fromEntries(counterparties.map(i => [i.id, i.name]));
    const mainAccountMap = Object.fromEntries(mainAccounts.map(i => [i.id, i.name]));
    const subAccountMap = Object.fromEntries(subAccounts.map(i => [i.id, i.name]));
    const subsidiaryMap = Object.fromEntries(subsidiaries.map(i => [i.id, i.name]));
